...
%
% CTO A.Y. 2025/26 project report — Donut Ranges in TAFFO VRA
...
% ===================== Metadata =====================
% The \title / \author / \date block below is kept for metadata only.
% The actual front matter uses a custom titlepage further down.
\title{Donut Ranges and Activation-Function Range Models for TAFFO}
\author{Mohamed Z. M. Mandour (10736813)}
\date{\today}
...
\includegraphics[width=0.45\linewidth]{logo/TAFFO-logo-black}

\vspace{1.8cm}

{\LARGE\bfseries Donut Ranges and\\[0.25em]
Activation-Function Range Models\\[0.25em]
for TAFFO's Value Range Analysis\par}
\vspace{0.8em}
{\large\itshape A combined implementation of CTO sub-projects \#1 and \#3\par}

\vspace{2cm}

{\large
\begin{tabular}{@{}r@{\quad}l@{}}
Author:      & Mohamed Z. M. Mandour \\
Person code: & 10736813 \\
Email:       & \texttt{mohamedzeyad.mandour@mail.polimi.it} \\
\end{tabular}\par}

\vspace{1.5cm}

{\large
Code Transformation and Optimization\\[0.2em]
Academic Year 2025/26\\[0.2em]
Prof.\ Giovanni Agosta\\[0.2em]
...
The contributions are evaluated on 85 unit-test assertions, a five-kernel
...
\item A standalone correctness \textbf{self-test} (28 assertions), an
  \textbf{arithmetic test} (57 assertions), a precision
  \textbf{micro-benchmark} (five kernels), and an
  \textbf{end-to-end benchmark} compiled by the full TAFFO driver.
\item A new \textbf{\texttt{scalar(range\_union(...))} annotation
  syntax} in TAFFO's Initializer (\S\ref{sec:range-union-syntax}),
  letting users declare donut ranges directly at source level, backed
  by an eight-assertion end-to-end driver-level smoke test
  (\S\ref{sec:range-union-eval}).
\item A \textbf{rigorous numerical certificate} for the non-monotonic
  activation minima: the GELU and SiLU $X_{\min,\mathrm{HI}}$,
  $X_{\min}$, $Y_{\min}$ constants are derived from a 60-decimal-digit
  \texttt{mpmath} computation double-checked on a 1000-point grid,
  pinned by five new unit-test assertions (\S\ref{sec:certified-constants}).
\item A \textbf{toy 1-hidden-layer MLP benchmark}
  (\S\ref{sec:mlp-benchmark}) exercising the full TAFFO pipeline on
  NN-shaped annotated code. It honestly reports the topological
  condition under which the donut refinement translates into DTA
  bit-width savings and when it does not.
\end{enumerate}
...
\item \textbf{Widening operators} (Cousot \& Cousot 1977; cf.\ History of AI p.~36): a binary
...
history paper~\cite{history-ai} that is part of the CTO reading list.
...
Cousot \& Cousot~(1977) and is reviewed in~\cite[p.~36]{history-ai}. We reuse
...
\texttt{test/donut\_ranges/donut\_arith\_test.cpp} &
  new arithmetic test (57 assertions) \\
...
\emph{certified} minimum constants (see
\S\ref{sec:certified-constants} below). The non-monotonic 1000-sample
scan that was in the repository before this project is deleted.

\subsection{Source-level annotation syntax: \texttt{range\_union(...)}}
\label{sec:range-union-syntax}

TAFFO's Initializer previously accepted only a single-interval
\texttt{scalar(range(a, b))} annotation. To let users declare donut
ranges directly at source level, we add a parallel form
\begin{center}
\texttt{scalar(range\_union((a1,b1),(a2,b2),...,(aN,bN)))}
\end{center}
to the recursive-descent parser in
\texttt{lib/TaffoInitializer/AnnotationParser.cpp}. The branch sits in
\texttt{parseScalar} \emph{before} the existing \texttt{range(...)}
branch --- the parser's \texttt{peek} primitive is prefix-based, so the
longer identifier must be tested first.

The new branch allocates a fresh \texttt{taffo::Range}, loops over
comma-separated \texttt{(lo, hi)} tuples, pushes each one directly
into \texttt{Range::components} (bypassing \texttt{addComponent}'s
\texttt{enableDonut}-off short-circuit so the seed carries the
component list regardless of VRA flag state), and then calls
\texttt{canonicalize()} and \texttt{rebuildHullFromComponents()}. A
single-component \texttt{range\_union} is legal and degenerates to a
classic range, which is useful when the annotation is generated by
tooling with variable arity.

With \texttt{-vra-donut-ranges} \emph{on}, VRA uses the seeded
components for every donut-aware handler it runs. With the flag
\emph{off}, the \texttt{components} field is still present in the
serialised metadata (so metadata written by a donut build can be
re-read by a classic build without loss), but VRA simply falls back
to the convex hull. This cross-mode interoperability is pinned by
the assertion harness in \S\ref{sec:range-union-eval}.

\subsection{Certified activation-minima constants}
\label{sec:certified-constants}

The \texttt{X\_MIN\_HI} / \texttt{X\_MIN} / \texttt{Y\_MIN} triples
that parametrise \texttt{nonMonotonicKernel} for GELU and SiLU were
hand-chosen in the first draft of the project. For the final
submission, those constants are derived from the Python script
\texttt{test/donut\_ranges/verify\_activation\_bounds.py}, which

\begin{itemize}[leftmargin=1.5em]
\item uses \texttt{mpmath} at 60-decimal-digit precision to locate the
  true minimum of each activation via \texttt{findroot} on the
  analytic derivative;
\item rounds the minimiser outward to the tightest 2-decimal
  sandwich $X_{\min,\mathrm{HI}} < x_{\min} < X_{\min}$;
\item rounds the exact minimum value down (more negative) and pads it
  with a $10^{-4}$ safety margin to produce the sound $Y_{\min}$;
\item validates the soundness by evaluating the activation on a dense
  1000-point grid across the bracket.
\end{itemize}

The certified constants are in Table~\ref{tab:certified-constants}.
Five dedicated unit tests in \texttt{donut\_arith\_test.cpp}
(\texttt{testGELUCertifiedMinimumStraddling},
\texttt{testGELUMonotoneDecreasingBranch},
\texttt{testSiLUCertifiedMinimumStraddling},
\texttt{testSiLUMonotoneIncreasingBranch},
\texttt{testGELUDonutPreservesHole}) pin this exact triple.

\begin{table}[H]
\centering
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Activation & $x_{\min}$ (60-digit)
& $X_{\min,\mathrm{HI}}$ & $X_{\min}$ & $Y_{\min}$ & slack \\
\midrule
GELU (tanh approx) & $-0.75246142207\ldots$ & $-0.76$ & $-0.75$
  & $-0.1702$ & $1.59\times 10^{-4}$ \\
SiLU / Swish & $-1.27846454276\ldots$ & $-1.28$ & $-1.27$
  & $-0.2786$ & $1.35\times 10^{-4}$ \\
\bottomrule
\end{tabular}
\caption{Certified constants for the non-monotonic activation
minima. Slack is $Y_{\min} - f(x_{\min})$: strictly more
negative than the true minimum, so downstream uses see a sound
over-approximation.}
...
... 16 sections, including 5 certified GELU / SiLU tests ...
Donut arithmetic test: 57 passed, 0 failed.
\end{lstlisting}

\textbf{Total: 85 assertions, all passing.}
...
\begin{lstlisting}[caption={End-to-end donut-range benchmark (excerpt).},label={lst:bench},basicstyle=\ttfamily\scriptsize]
...
\subsection{End-to-end test of the \texttt{range\_union} annotation syntax}
\label{sec:range-union-eval}

The donut annotation parser added in \S\ref{sec:range-union-syntax}
is exercised by \texttt{test/donut\_ranges/donut\_annotated.c}, a
self-contained benchmark in which two globals are seeded with
\texttt{scalar(range\_union((-0.8,\,-0.2),(0.2,\,0.8))) target('bimodal\_w')}
and \texttt{... target('bimodal\_w2')}. A kernel reads both globals and
computes $w^2$ (exercising \texttt{handleMul}'s self-square fast
path) and $w\cdot w'$ (exercising the two-donut Cartesian product).
The driver script \texttt{test/donut\_ranges/run\_donut\_annotated.sh}
compiles the file twice --- once with \texttt{-Xvra -vra-donut-ranges}
and once without --- and runs eight Python assertions against the
generated \texttt{taffo\_info\_vra.json}.
...
\# & Assertion & ON & OFF \\
\midrule
1 & \texttt{bimodal\_w} VRA seed carries
    \texttt{[[-0.8,-0.2],[0.2,0.8]]} & $\checkmark$ & $\checkmark$ \\
2 & \texttt{bimodal\_w2} VRA seed carries the same components
    & $\checkmark$ & $\checkmark$ \\
3 & $w*w$ collapses to tight $[0.04, 0.64]$ (self-square fast path)
    & $\checkmark$ & --- \\
4 & $w*w'$ preserves two-component donut
    $[-0.64,-0.04] \cup [0.04, 0.64]$ & $\checkmark$ & --- \\
5 & seed components survive serialisation in a classic build
    & --- & $\checkmark$ \\
6 & classic build falls back to hull $[0, 0.64]$ for $w*w$
    & --- & $\checkmark$ \\
7 & classic build falls back to hull $[-0.64, 0.64]$ for $w*w'$
    & --- & $\checkmark$ \\
8 & driver completes with exit code 0
    & $\checkmark$ & $\checkmark$ \\
\bottomrule
\end{tabularx}
\caption{Assertions checked by
\texttt{test/donut\_ranges/run\_donut\_annotated.sh}. All eight pass
on the reference machine.}
\label{tab:range-union-assertions}
\end{table}

This harness confirms that (i) the parser populates
\texttt{components} correctly from source text, (ii) the list
propagates through VRA into the emitted metadata, (iii) downstream
arithmetic uses the donut structure when the flag is on, and (iv) the
exact same annotation silently degrades to the convex hull when the
flag is off --- the promised backwards-compatibility property of
\S\ref{sec:contributions} item~5.

\subsection{Toy MLP end-to-end benchmark}
\label{sec:mlp-benchmark}

\texttt{test/donut\_ranges/donut\_mlp.c} is a 1-hidden-layer MLP
with $\text{in}=4$, $\text{hidden}=8$, $\text{out}=1$. First-layer
weights $W_1$ and second-layer weights $W_2$ are seeded with the new
\texttt{range\_union} syntax. Biases and inputs carry ordinary
single-interval annotations. The activation is
\texttt{taffo\_relu}, exercising the donut-aware
\texttt{handleCallToReLU}. The driver script
\texttt{run\_donut\_mlp.sh} runs TAFFO twice and prints a table of
(VRA range, DTA fixed-point format) per annotated SSA group; the
table is reproduced verbatim in Table~\ref{tab:mlp-benchmark}.
...
\texttt{W1} & $[-0.8, 0.8]$
  ...
  & \texttt{s2\_30fixp} & \texttt{s2\_30fixp} \\
\texttt{W2} & $[-0.6, 0.6]$
  & $[-0.6, 0.6]$ + \{[-0.6,-0.15],[0.15,0.6]\}
  & \texttt{s4\_28fixp} & \texttt{s4\_28fixp} \\
\texttt{b1} & $[-0.5, 0.5]$ & $[-0.5, 0.5]$
  & \texttt{s2\_30fixp} & \texttt{s2\_30fixp} \\
\texttt{b2} & $[-0.3, 0.3]$ & $[-0.3, 0.3]$
  & \texttt{s3\_29fixp} & \texttt{s3\_29fixp} \\
\texttt{x}  & $[-1, 1]$ & $[-1, 1]$
  & \texttt{s2\_30fixp} & \texttt{s2\_30fixp} \\
\texttt{h\_pre}  & $[-3.7, 3.7]$ & $[-3.7, 3.7]$
  & \texttt{s4\_28fixp} & \texttt{s4\_28fixp} \\
\texttt{h\_post} & $[-3.7, 3.7]$ & $[-3.7, 3.7]$
  & \texttt{s4\_28fixp} & \texttt{s4\_28fixp} \\
\texttt{y\_pre}  & $[-4.92, 4.92]$ & $[-4.92, 4.92]$
  & \texttt{s4\_28fixp} & \texttt{s4\_28fixp} \\
\texttt{y\_post} & $[-2.7, 2.7]$ & $[-2.7, 2.7]$
  & \texttt{s3\_29fixp} & \texttt{s3\_29fixp} \\
\bottomrule
\end{tabular}
\caption{Donut versus classic VRA ranges and DTA formats on the toy
MLP. The seeded weights $W_1, W_2$ carry donut components through VRA
in the donut build; derived quantities lose the hole because the
ReLU + fan-in-sum topology fuses the components.}
\label{tab:mlp-benchmark}
\end{table}

Two observations.

\begin{enumerate}[leftmargin=1.5em]
\item Seeds survive: $W_1$ and $W_2$ carry their donut components in
  the VRA JSON of the donut build and lose them in the classic
  build --- confirming that the annotation parser and serialisation
  path work as advertised.
\item Derived quantities collapse: each pre-activation
  $h_j = \sum_i W_{j,i} x_i + b_j$ multiplies a \emph{bimodal} weight
  by a zero-crossing $x_i \in [-1, 1]$. The Cartesian product
  $[-0.8,-0.2] \cdot [-1,1] = [-0.8, 0.8]$ already destroys the hole
  at the first multiplication; the fan-in sum and the subsequent ReLU
  never re-introduce it.
\end{enumerate}

The finding is that donut ranges pay off \emph{where a
zero-excluding denominator or a self-square sits between a
donut-shaped input and DTA}, as demonstrated by
\texttt{donut\_array.c} in \S\ref{sec:eval-e2e} (+24 fractional bits
on the reciprocal of a bimodal weight). In a plain ReLU-MLP the
bimodal structure of the weights is destroyed by a standard
fully-connected pre-activation before it can reach DTA. This is the
same observation that motivates the thesis-extension follow-up of
\S\ref{sec:limit-dta} --- teaching DTA to consume the
\texttt{components} field at the intermediate stages, before the
hole closes in a full NN topology.

\subsection{Discussion: what a full MNIST MLP would show}
...
The toy MLP of \S\ref{sec:mlp-benchmark} already demonstrates that
the TAFFO pipeline processes a full NN forward pass with donut
...
DTA. The array-based benchmark of \S\ref{sec:eval-e2e} is
deliberately minimal because it targets the single pipeline stage
where the gain originates --- VRA's division fast path --- and
demonstrates the full flow-through of that gain into DTA's bit-width
decision without being obscured by the hundreds of other arithmetic
instructions in a full MLP forward pass.
...
\subsection{Annotation syntax (resolved)}
...
delivers the
\texttt{scalar(range\_union((a1, b1), (a2, b2), ...))} form
(\S\ref{sec:range-union-syntax}) and exercises it end-to-end with the
eight-assertion smoke test of \S\ref{sec:range-union-eval}. The only
remaining concern in this area is the \emph{interaction} between
\texttt{range\_union} seeds and TAFFO's downstream passes: DTA still
reads only the convex hull, so a \texttt{range\_union} seed whose
hull crosses zero still feeds a wide interval into
\texttt{handleMul}'s Cartesian product when multiplied by a
zero-crossing operand (see the MLP analysis in
\S\ref{sec:mlp-benchmark}). Making DTA a donut consumer
(\S\ref{sec:limit-dta}) completes the picture.
...
\subsection{Soundness of the constants for GELU / SiLU (resolved)}

In the first draft this subsection was a limitation: the GELU and
SiLU minimum constants were hand-chosen and had no formal
certificate. The final submission replaces them with mpmath-certified
values obtained by
\texttt{test/donut\_ranges/verify\_activation\_bounds.py} at 60-digit
precision, double-checked on a 1000-point grid, and pinned by five
new unit-test assertions. Full details are in
\S\ref{sec:certified-constants}; the certified triples are in
Table~\ref{tab:certified-constants}.

The only residual concern is the approximation error of the GELU
$\tanh$ formula versus the exact GELU that uses the error function
$\operatorname{erf}$. Implementations that target the exact GELU can
use the same certificate machinery by supplying a different
$(f, f')$ pair to \texttt{certify(...)}.
...
\subsection{Conversion-pass fixes uncovered by the MLP benchmark (fixed)}
\label{sec:limit-convfix}

Putting the toy MLP of \S\ref{sec:mlp-benchmark} through the full
TAFFO driver revealed two pre-existing defects in the Conversion
pass, both unrelated to donut ranges but both blocking any user
whose source looks like a standard neural network. Both are fixed
in this submission.

\begin{enumerate}
\item \textbf{\texttt{convertStore} missing insertion point.} The
  store-conversion handler passed \texttt{nullptr} as the
  insertion-point argument when asking \texttt{getConvertedOperand}
  to coerce the store's value into the target fixed-point format.
  When the value operand is a function argument or a constant (not
  an \texttt{Instruction}), \texttt{genConvertConvToConv} cannot
  derive an insertion point and asserted
  \texttt{insertionPoint required}. Passing the \texttt{store}
  instruction as insertion point --- the same anchor the store's
  own \texttt{IRBuilder} uses a few lines below --- fixes the crash
  and is the natural semantic choice.
\item \textbf{Float-typed source with fixed-point metadata in
  \texttt{genConvertConvToConv}.} Cloned functions briefly carry a
  function argument whose LLVM type is still \texttt{double} but
  whose TAFFO \texttt{ConversionType} metadata has already been
  updated to fixed-point in preparation for the body walk. In that
  state \texttt{srcConvType.isFloatingPoint()} is false even though
  \texttt{src->getType()->isFloatingPointTy()} is true, so the
  existing branch that routes float~$\to$~fixed-point through
  \texttt{genConvertFloatToConv} did not trigger and the integer
  \texttt{CreateSExtOrTrunc} path crashed on a \texttt{double}
  operand. The fix widens that guard to trust the actual LLVM type
  as well as the metadata.
\end{enumerate}

With both fixes in place the toy MLP of \S\ref{sec:mlp-benchmark}
runs through all five TAFFO pass stages (TypeDeducer, Initializer,
VRA, DTA, Conversion) and the final \texttt{.taffotmp.s} assembly
artefact is produced, confirming that the full pipeline is
operational on NN-shaped annotated code.
...
Sub-projects \#8--\#10 in the CTO 2025/26 project list address the
...
zero runtime cost. The principal remaining work is on the
...
coverage of NN workloads beyond the toy MLP of~\S\ref{sec:mlp-benchmark}
is a natural thesis-scope extension.
...
\bibitem{cot-ir}
G.~Agosta. \emph{CTO Slides 2 --- Intermediate Representation}. Code
Transformation and Optimization, Politecnico di Milano, A.Y.\ 2025/26.
\texttt{lectures/Slides/COT\_slides\_2\_IR.pdf}.

\bibitem{cot-dataflow}
G.~Agosta. \emph{CTO Slides 5 --- Dataflow Analysis}. Code
Transformation and Optimization, Politecnico di Milano, A.Y.\ 2025/26.
...
G.~Agosta et al. \emph{TAFFO project proposals, CTO A.Y.\ 2025/26}.
...
# Paths: set these once, everything below uses them.
export TAFFO=$HOME/TAFFO         # adjust to your checkout
export LLVM=/opt/homebrew/opt/llvm@18/lib/cmake/llvm
export BLD=$TAFFO/cmake-build-debug
export T=$BLD/test/donut_ranges

# 1. Configure and build the three test binaries.
cd $TAFFO
cmake -B $BLD -S . -DLLVM_DIR=$LLVM
cmake --build $BLD -j \
      --target donut_range_selftest donut_arith_test donut_microbench

$T/donut_range_selftest
$T/donut_arith_test
$T/donut_microbench

# 2. Build the full Taffo plugin and install it.
cmake --build $BLD --target Taffo -j
cp $BLD/lib/Taffo.dylib $TAFFO/install/lib/Taffo.dylib

# 3. Run the end-to-end benchmark twice, keeping temp dirs.
rm -rf /tmp/classic /tmp/donut
mkdir -p /tmp/classic /tmp/donut

$TAFFO/install/bin/taffo -O2 -c -emit-llvm \
    -temp-dir /tmp/classic -o /tmp/classic.ll \
    $TAFFO/test/donut_ranges/donut_array.c

$TAFFO/install/bin/taffo -O2 -c -emit-llvm \
    -Xvra -vra-donut-ranges \
    -temp-dir /tmp/donut -o /tmp/donut.ll \
    $TAFFO/test/donut_ranges/donut_array.c

# 4. Inspect the final DTA bit-widths (look for sNN_MMfixp suffixes).
grep -o 's[0-9]*_[0-9]*fixp' /tmp/classic.ll | sort -u
grep -o 's[0-9]*_[0-9]*fixp' /tmp/donut.ll   | sort -u

# 5. Run the range_union annotation smoke test (eight assertions).
$TAFFO/test/donut_ranges/run_donut_annotated.sh

# 6. Run the toy MLP benchmark (classic vs donut VRA and DTA table).
$TAFFO/test/donut_ranges/run_donut_mlp.sh

# 7. Re-run the mpmath certificate for the activation minima.
python3 $TAFFO/test/donut_ranges/verify_activation_bounds.py
\end{lstlisting}

The mpmath certificate requires the \texttt{mpmath} PyPI package.
All other steps rely only on CMake, Ninja (via CMake's generator
selection), the Homebrew-provided LLVM~18, and Python~3. On the
reference machine (Apple Silicon MacBook Pro, macOS 26.4, Homebrew
LLVM 18.1.8) the full reproduction pipeline takes under two
minutes; the slowest step is the initial CMake configure.
