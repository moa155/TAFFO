...
% PoliMI logo placeholder. Upload `polimi-logo.pdf` (or .png) to the
% Overleaf project alongside report.tex. If the file is missing the
% fallback box is drawn instead, so the document still compiles.
\IfFileExists{polimi-logo.pdf}
  {\includegraphics[width=0.35\linewidth]{polimi-logo}}
  {\IfFileExists{polimi-logo.png}
    {\includegraphics[width=0.35\linewidth]{polimi-logo}}
    {\fbox{\parbox[c][3.5cm][c]{0.35\linewidth}{\centering\itshape\small
       upload \texttt{polimi-logo.pdf}\\or \texttt{polimi-logo.png}\\to Overleaf}}}}
