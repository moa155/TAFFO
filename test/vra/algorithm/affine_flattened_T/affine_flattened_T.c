...
#if OLDVRA
float data[R][C] __attribute__((annotate("scalar(range(" PB_XSTR(RMIN) ", " PB_XSTR(RMIN) "))")));
float data2[R][C] __attribute__((annotate("scalar(range(" PB_XSTR(RMIN) ", " PB_XSTR(RMIN) "))")));
float dest[R][C] __attribute__((annotate("scalar(range(-75000,150000))")));
#else
float data[R][C] __attribute__((annotate("scalar(range(0,0))")));
float data2[R][C] __attribute__((annotate("scalar(range(0,0))")));
float dest[R][C] __attribute__((annotate("scalar(range(0,0))")));
#endif
...
int main(int argc, char const *argv[])
{
    ...
    for (int m = 0; m < M; ++m) {
        ...
        for (int i = 0; i < R; i++) {
            for (int j = 0; j < C; j++) {
                dest[i][j] = rand_range(RMIN, RMAX);    //init
            }
        }
